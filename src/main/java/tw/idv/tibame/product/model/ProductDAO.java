...
public class ProductDAO implements ProductDAO_interface {
	...
	public ProductVO indexValue(Integer p_m_id) {
		...
		try {
			...
			while (rs.next()) {
				productVO = new ProductVO.Builder().setA(rs.getInt(1)).setB(rs.getInt(2)).setC(rs.getInt(3))
						.setD(rs.getInt(4)).build();
			}
	...
	public List<MasterPicVO> indexNatrix1(Integer p_m_id) {
		...
		try {
			...
			while (rs.next()) {
				...
				masterPicVO = new MasterPicVO.Builder().setP_name(rs.getString(1)).setB(rs.getInt(2)).setC(rs.getInt(3))
						.setP_pic_one(p1).build();
